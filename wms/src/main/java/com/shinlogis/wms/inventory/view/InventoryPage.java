...
public class InventoryPage extends Page {
    ...
    public InventoryPage(AppMain appMain) {
        ...
        tblPlan.getColumnModel().getColumn(editColumnIndex).setCellEditor(new ButtonEditor(new JCheckBox(), (table, row, column) -> {
        ...
        tblPlan.getColumnModel().getColumn(deleteColumnIndex).setCellEditor(new ButtonEditor(new JCheckBox(), (table, row, column) -> {
            int modelRow = tblPlan.convertRowIndexToModel(row);
            InventoryDTO dto = fullList.get((currentPage - 1) * rowsPerPage + modelRow);

            int confirm = javax.swing.JOptionPane.showConfirmDialog(this, "재고를 삭제하시겠습니까?", "삭제 확인", javax.swing.JOptionPane.YES_NO_OPTION);
            if (confirm == javax.swing.JOptionPane.YES_OPTION) {
                InventoryDAO dao = new InventoryDAO();
                boolean success = false;
                
                if (dto.getInventoryId() > 0) {
                    // inventory_id가 있으면 개별 삭제
                    success = dao.deleteInventory(dto.getInventoryId());
                } else {
                    // inventory_id가 없으면 병합 삭제
                    success = dao.deleteInventoryMerged(dto);
                }
    ...
    private void loadInventoryData() {
        ...
        if (chooser.getDate() != null) {
            ...
            inventoryDTO.setExpiryDate(null);
        }
