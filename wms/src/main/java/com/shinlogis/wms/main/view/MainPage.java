...
public class MainPage extends Page {
    ...
    private JPanel pTop;
    private JPanel pt1, pt2, pt3, pt4, pt5;
    private JLabel pt1Value, pt2Value, pt3Value, pt4Value, pt5Value;
    ...
    public MainPage(AppMain appMain) {
        ...
        StatPanel stat1 = createStatPanel("오늘 입고예정", Integer.toString(statisticDAO.getTotalInboundToday()), new Color(72, 145, 220));
        pt1 = stat1.panel;
        pt1Value = stat1.valueLabel;

        StatPanel stat2 = createStatPanel("오늘 입고완료", Integer.toString(statisticDAO.getTotalInboundCompleted()), new Color(60, 120, 200));
        pt2 = stat2.panel;
        pt2Value = stat2.valueLabel;

        StatPanel stat3 = createStatPanel("오늘 출고예정", Integer.toString(statisticDAO.getTotalOutboundToday()), new Color(220, 72, 72));
        pt3 = stat3.panel;
        pt3Value = stat3.valueLabel;

        StatPanel stat4 = createStatPanel("오늘 출고완료", Integer.toString(statisticDAO.getTotalOutboundCompleted()), new Color(200, 60, 60));
        pt4 = stat4.panel;
        pt4Value = stat4.valueLabel;

        StatPanel stat5 = createStatPanel("오늘 출고금액", Integer.toString(statisticDAO.getTotalOutboundCompletedPrice()) + " ₩", new Color(72, 220, 128));
        pt5 = stat5.panel;
        pt5Value = stat5.valueLabel;
        ...
        DefaultListModel<String> inboundModel = new DefaultListModel<>();
        for (IODetail detail : inboundList) {
            inboundModel.addElement(String.format("[입고 #%d] %s - %d개",
                    detail.getIoReceipt().getIoReceiptId(),
                    detail.getProductSnapshot().getProductName(),
                    detail.getActualQuantity()));
        }
    ...
    private static class StatPanel {
        JPanel panel;
        JLabel valueLabel;

        StatPanel(JPanel panel, JLabel valueLabel) {
            this.panel = panel;
            this.valueLabel = valueLabel;
        }
    }

    private StatPanel createStatPanel(String title, String value, Color color) {
        JPanel panel = new RoundedPanel(new BorderLayout(5, 5), 20, color);
        panel.setBorder(BorderFactory.createEmptyBorder(15, 15, 15, 15));
        panel.setPreferredSize(new Dimension(150, 100));
        ...
        return new StatPanel(panel, valueLabel);
    }
    ...
    public void refreshData() {
        ...
        pt2Value.setText(Integer.toString(statisticDAO.getTotalInboundCompleted()));
        pt3Value.setText(Integer.toString(statisticDAO.getTotalOutboundToday()));
        pt4Value.setText(Integer.toString(statisticDAO.getTotalOutboundCompleted()));
        pt5Value.setText(Integer.toString(statisticDAO.getTotalOutboundCompletedPrice()) + " ₩");
    }
}
